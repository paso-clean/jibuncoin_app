...
if($url_param[1] == 'ajax' || $url_param[1] == 'api'){
	if(isset($url_param[2]) || isset($url_param[3])){
		if($url_param[2] == 'common'){
		    ...
		    $require_file = XSYSTEM_COMMON_DIR . $url_param[1] . '/' . $url_param[2] . '/' . $url_param[1] . '-' . $url_param[2] . '-' . $url_param[3] . '.php';
		}
	}else{
	    $require_file = XSYSTEM_COMMON_DIR . 'page/page-error-404.php';
	}
}elseif(!isset($url_param[1]) || $url_param[1] == ''){
    setcookie("app_name",$url_param[0],time()+60*60*24*30,APP_URI);
    $require_file = XSYSTEM_COMMON_DIR . 'page/page-index.php';
}else{
    setcookie("app_name",$url_param[0],time()+60*60*24*30,APP_URI);

	if(!isset($url_param[2]) || $url_param[2] == ''){
	    $require_file = XSYSTEM_COMMON_DIR . 'page/' . $url_param[1] . '/' . 'page-' . $url_param[1] . '-index.php';

	}else{
	    $require_file = XSYSTEM_COMMON_DIR . 'page/' . $url_param[1] . '/' . 'page-' . $url_param[1] . '-' . $url_param[2] . '.php';
	}
}
