...
class Network extends Entity {
	...
	function get_active_networks($network_type = ''){
	    if($network_type != ''){
	        $where = "AND network_type = '" . $network_type . "'";
	    }else{
	        $where = '';
	    }
	    $query = "SELECT * FROM xsystem_networks WHERE active = 1 " . $where . " ORDER BY updated_at";
	    $results = Db::$db->query($query);
	    $networks = array();
	    while($data = $results->fetch_assoc()){
	        $networks[] = $data;
	...
	function add_network($post){
	    ...
	    $network_key = xsystem_random_code(30);
	...
	function get_network_servers($network_code){
	    $query = "SELECT * FROM " . XSYSTEM_PRODUCT . "_networks AS nw
	...
	function get_network_base_server($network_type,$network_code,$uri){
	    $network_servers = $this->get_network_servers($network_code);
	    foreach($network_servers as $network_server){
	        if($network_server['server_type'] == $network_type){
	            $servers[] = $network_server;
	            if(strpos($network_server['domain'],$uri) === 0){
	                $base_server = $network_server;
	            }
	        }
	    }
	    
	    if(!isset($base_server) && count($network_server) != 0){
	        $base_server = $network_server[0];
	    }
	    
	    if(!isset($base_server)){
	        return false;
	    }
