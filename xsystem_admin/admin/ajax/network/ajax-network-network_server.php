...
$network_servers = $networkIns->get_network_servers($network_code);
...
foreach($servers as $server){
    ...
    foreach($network_servers as $network_server){
        if($server['server_code'] == $network_server['server_code']){
            $is_network_server = true;
        }
    }
    if($server['server_type'] == $network_type && !$is_network_server){
